...
describe('SudokuCell', () => {
    ...
    it('auto-calculates block number from row and column numbers', () => {
        assert.equal(SudokuCell({row: 0, column: 0, value: 3}).block, 0);
        assert.equal(SudokuCell({row: 4, column: 3, value: 9}).block, 4);
        assert.equal(SudokuCell({row: 8, column: 8, value: 1}).block, 8);
    });

    describe('fromMatrixRowNum', () => {
        it('creates the sudoku cell that maps to the nth row of the DLX matrix', () => {
            const firstRowCell = SudokuCell({row: 0, column: 0, value: 1});
            assert.deepStrictEqual(SudokuCell.fromMatrixRowNum(0), firstRowCell);

            const middleRowCell = SudokuCell({row: 2, column: 3, value: 4});
            assert.deepStrictEqual(SudokuCell.fromMatrixRowNum(81*2+9*3+4-1), middleRowCell);

            const lastRowCell = SudokuCell({row: 8, column: 8, value: 9});
    ...
    describe('cellConstraintColNum', () => {
        it('returns the DLX matrix column number corresponding to the "one number in cell" constraint', () => {
            ...
            const middleRowCell = SudokuCell({row: 2, column: 3, value: 5});
